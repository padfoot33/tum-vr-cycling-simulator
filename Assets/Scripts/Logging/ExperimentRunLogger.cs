...
{
    /// <summary>
    /// 20 Hz experiment CSV.
    /// Includes wall-clock timestamps for EdaMove / Movesense sync.
    /// </summary>
    ...
    public class ExperimentRunLogger : MonoBehaviour
    {
        ...
        [Tooltip("Only severe FPS drops are marked as technical issues. Raw FPS is always logged.")]
        [SerializeField] private float severeTechnicalIssueFps = 15f;
        ...
        private BicycleSimulatorController _bikeController;
        ...
        public void BindRefs()
        {
            ...
            {
                if (bikeTransform == null)
                    bikeTransform = refs.bicycleTransform;
            ...
            // NEW:
            // Find the simulator controller so we can log
            // Wahoo target speed and power.
            if (bikeTransform != null)
            {
                _bikeController =
                    bikeTransform.GetComponent<BicycleSimulatorController>();

                if (_bikeController == null)
                {
                    _bikeController =
                        bikeTransform.GetComponentInParent<BicycleSimulatorController>();
                }
            }
        }
            ...
            string taskContext = "approach")
        {
            ...
            {
                Debug.LogError(
                    "[ExperimentRunLogger] Bike Transform is not assigned."
                );
            ...
            _taskContext =
                string.IsNullOrEmpty(taskContext)
                    ? "approach"
                    : taskContext;
            ...
            _runId =
                $"{safeParticipant}_{safeScenario}_{timestamp}";
            ...
            _writer.WriteLine(
                "run_id,participant_id,scenario_name,condition_id,traffic_enabled,segment_id,task_context,t,timestamp_utc,unix_time_ms,event,event_phase,x,y,z,speed_kph,target_speed_kph,drive_input,accel_curve_output,current_top_speed,configured_top_speed,vel_x,vel_z,heading_deg,movement_dir_deg,yaw_rate_deg_s,steering_angle_deg,brake_left,brake_right,brake_active,deviation_from_path,event_vehicle_x,event_vehicle_z,event_vehicle_speed_kph,frame_time_ms,fps,technical_issue_flag"
            );
            ...
            Debug.Log(
                $"[ExperimentRunLogger] Logging started: {_runFilePath}"
            );
        }
        ...
        public void StopLogging()
        {
            ...
            Debug.Log(
                "[ExperimentRunLogger] Logging stopped."
            );
        }
        ...
        public void UpdateEventVehicleData(
            float x,
            float z,
            float speedKph)
        {
        ...
        private void TryWriteSyncMarker(
            string markerName)
        {
            if (!_isLogging)
            {
                Debug.LogWarning(
                    $"[ExperimentRunLogger] {markerName} ignored: logging is not running."
                );
            ...
            Debug.Log(
                $"[ExperimentRunLogger] {markerName} unix_time_ms={unixMs}"
            );
        }
        ...
        private void BindReferencePathForScenario()
        {
            ...
            if (referencePathTracker != null &&
                bikeTransform != null)
            {
                referencePathTracker.bikeTransform =
                    bikeTransform;
            }
        }
        ...
        {
            var refs =
                ExperimentSceneRefs.Instance;

            if (ExperimentBuildSession.IsActive)
            {
                _trafficEnabled =
                    ExperimentBuildSession.TrafficEnabled
                        ? 1
                        : 0;
            }
            else if (
                refs != null &&
                refs.cityTraffic != null)
            {
                _trafficEnabled =
                    refs.cityTraffic.IsTrafficEnabled
                        ? 1
                        : 0;
            }
            else
            ...
            if (IsRoute2(_scenarioName))
            {
                ...
                !string.IsNullOrEmpty(_scenarioName) &&
                _scenarioName.IndexOf(
                    "Route1",
        ...
        private static bool IsRoute2(
            string scenarioName)
        {
            return
                !string.IsNullOrEmpty(scenarioName) &&
                scenarioName.IndexOf(
                    "Route2",
                    StringComparison.OrdinalIgnoreCase
                ) >= 0;
        }
        ...
        private string CurrentEventName()
        {
            ...
            if (
                !string.IsNullOrEmpty(_closePassEvent) &&
                !string.Equals(
                    _closePassEvent,
                    "NONE",
            ...
            bool updateKinematics)
        {
            ...
            float movementDirDeg =
                _lastMovementDirDeg;

            float yawRateDegS =
                _lastYawRateDegS;

            float steeringRateDegS =
                _lastSteeringRateDegS;

            if (updateKinematics)
            {
                float dt =
                    (float)(now - _lastSampleUtc)
                    .TotalSeconds;
                ...
                // FINAL FIX:
                // Cycling speed is horizontal X/Z speed only.
                Vector3 flatDelta =
                    new Vector3(
                        delta.x,
                        0f,
            ...
            GetBrakeInputs(
                out float brakeLeft,
                out float brakeRight,
                out int brakeActive
            );

            // Clamp tiny negative sensor offset values.
            brakeLeft =
                Mathf.Max(
                    0f,
                    brakeLeft
...
float.NaN;
            ...
            {
                driveInput =
                    _bikeController.customAccelerationAxis;
            }
            float accelCurveOutput =
    float.NaN;

            float currentTopSpeed =
                float.NaN;
           
            float configuredTopSpeed =
    float.NaN;

            if (_bikeController != null)
            {
                accelCurveOutput =
                    _bikeController.accelerationCurve != null
                        ? _bikeController.accelerationCurve.Evaluate(
                            _bikeController.customAccelerationAxis
                        )
                        : float.NaN;

                currentTopSpeed =
                    _bikeController.currentTopSpeed;
                configuredTopSpeed =
    _bikeController.topSpeed;
            }
...
// Actual Unity bicycle speed
F(_smoothedSpeedKph),
FV(targetSpeedKph),
FV(driveInput),
FV(accelCurveOutput),
FV(currentTopSpeed),
FV(configuredTopSpeed),
...
F(velocityWorld.z),
            ...
            out int active)
        {
            ...
            left =
                _cyclist.GetLeftBrake();

            right =
                _cyclist.GetRightBrake();

            active =
                _cyclist.IsBrakeActive()
                    ? 1
                    : 0;
        }
        ...
        private void WriteMetadataBlock()
        {
            ...
            _writer.WriteLine(
                $"# Traffic Enabled: {_trafficEnabled}"
            );

            _writer.WriteLine(
                $"# Initial Segment ID: {_segmentId}"
            );

            _writer.WriteLine(
                $"# Initial Task Context: {_taskContext}"
            );

            _writer.WriteLine(
                "# Initial Event Phase: NONE"
            );
            ...
            _writer.WriteLine(
                $"# Path Deviation Enabled: {referencePathTracker != null}"
            );

            _writer.WriteLine(
                "# Event Vehicle Enabled: True"
            );

            _writer.WriteLine(
                "# Wahoo Target Speed Enabled: True"
            );

            _writer.WriteLine(
                "# Wahoo Power Enabled: True"
            );

            _writer.WriteLine(
                $"# Log Frequency (Hz): {logHz}"
            );
            ...
            _writer.WriteLine(
                $"# Simulation Start Local: {DateTime.Now:yyyy-MM-dd HH:mm:ss.fff}"
            );

            _writer.WriteLine(
                $"# Simulation Start UTC: {_runStartUtc.UtcDateTime:yyyy-MM-ddTHH:mm:ss.fffZ}"
            );

            _writer.WriteLine(
                $"# Simulation Start Unix MS: {_runStartUtc.ToUnixTimeMilliseconds()}"
            );

            _writer.WriteLine(
                $"# Log Path: {_runFilePath}"
            );
        ...
        internal static string SanitizeParticipantId(
            string id)
        {
        ...
        internal static string StepParticipantId(
            string id,
            int delta)
        {
            id =
                SanitizeParticipantId(id);
