...
// Cycle Geometry Class - Holds Gameobjects pertaining to the specific bicycle
    ...
    public class BicycleController : MonoBehaviour, IVehicleController 
    {
        ...
        public float globalSpeedGainSimBike = 1.5f;
        ...
        void Awake()
        {
            ...
            Debug.Log($"[BicycleController] 🎯 AWAKE: Scene='{gameObject.scene.name}' | GameObject='{gameObject.name}' | Path='{parentPath}'");
        ...
        {
            string parentPath = GetFullHierarchyPath();
            Debug.Log($"[BicycleController] 🚀 START: Scene='{gameObject.scene.name}' | GameObject='{gameObject.name}' | Path='{parentPath}' | Active={gameObject.activeInHierarchy}");
        ...
        public void DisableSumoTeleport(string reason)
        {
            ...
            Debug.Log($"[BicycleController] 🔐 SUMO teleport DISABLED ({sumoTeleportDisableCount}x): {reason}");
        }
        ...
        public void EnableSumoTeleport(string reason)
        {
            ...
            if (sumoTeleportDisableCount == 0)
            {
                ...
                Debug.Log($"[BicycleController] 🔓 SUMO teleport ENABLED: {reason}");
            }
            else
            {
                Debug.Log($"[BicycleController] ⏱️ SUMO teleport still disabled ({sumoTeleportDisableCount} nested): {reason}");
            }
        }
        ...
        public void DisableWayPointPlayback(string reason)
        {
            ...
            Debug.Log($"[BicycleController] 🔐 WayPoint playback DISABLED ({wayPointPlaybackDisableCount}x): {reason}");
        }
        ...
        public void EnableWayPointPlayback(string reason)
        {
            ...
            if (wayPointPlaybackDisableCount == 0)
            {
                ...
                Debug.Log($"[BicycleController] 🔓 WayPoint playback ENABLED: {reason}");
            }
            else
            {
                Debug.Log($"[BicycleController] ⏱️ WayPoint playback still disabled ({wayPointPlaybackDisableCount} nested): {reason}");
            }
        }
        ...
        {
            string parentPath = GetFullHierarchyPath();
            Debug.Log($"[BicycleController] ✅ ONENABLE: Scene='{gameObject.scene.name}' | GameObject='{gameObject.name}' | Path='{parentPath}'");
        ...
        void ApplyCustomInput()
        {
            ...
            if (wayPointSystem.recordingState == WayPointSystem.RecordingState.DoNothing || wayPointSystem.recordingState == WayPointSystem.RecordingState.Record)
            {
                if(isSumoVehicle){
                    ...
                    {
                        // Only teleport if spawn has completed or minimum frames have passed
                        ...
                        {
                            // Waiting for ScenarioSpawnApplier to complete
                            customSteerAxis = 0;
                            customLeanAxis = 0;
                            customAccelerationAxis = 0;
                            rawCustomAccelerationAxis = 0;
                ...
                else if (isSimulatorVehicle) 
                {
                    ...
                    // torque input
                    float torqueInput = piControllerSpeedWahoo.Control(target_velocity_wahoo_bike, actual_velocity_unity_bike);

                    if (target_velocity_wahoo_bike < 0.1f)
                    {
