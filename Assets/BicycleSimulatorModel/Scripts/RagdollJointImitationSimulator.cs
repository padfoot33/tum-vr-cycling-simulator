...
public class RagdollJointImitationSimulator : MonoBehaviour
{

    private string limbTag = "TrackedLimb";
    GameObject CharacterCopyPosition;
    GameObject CharacterCopyAnimations;
    BicycleSimulatorController bicycleController;
    ...
    void Start()
    {
        bicycleController = FindObjectOfType<BicycleSimulatorController>();
        ...
        Debug.Log($"[RagdollJointImitationSimulator] Start() - BicycleController position: {bicycleController.transform.position}, Y={bicycleController.transform.position.y}");
        Debug.Log($"[RagdollJointImitationSimulator] Start() - CharacterCopyPosition: {CharacterCopyPosition.transform.position}, Y={CharacterCopyPosition.transform.position.y}");
        ...
        Debug.Log($"[RagdollJointImitationSimulator] After positioning - RagdollJoint position: {transform.position}, Y={transform.position.y}");
    ...
    private void AddJointFollowScript()
    {
        ...
        {
            cj.gameObject.AddComponent<RagdollJointConfigSimulator>();
            ...
            for (int t = 0; t < allAnimTrans.Length; t++)
            {
                ...
                {
                    cj.GetComponent<RagdollJointConfigSimulator>().torqueForce = Stiffness;
                    cj.GetComponent<RagdollJointConfigSimulator>().angularDamping = jointAngularDamping;
                    cj.GetComponent<RagdollJointConfigSimulator>().target = allAnimTrans[t];
                }
            }
        }
    }
