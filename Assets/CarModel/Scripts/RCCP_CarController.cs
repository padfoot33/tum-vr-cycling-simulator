...
public class RCCP_CarController : MonoBehaviour, IVehicleController {

    public string id { get; set; } // SUMO Identifiziert in Vehicle Dictionary
    ...
    public bool isSumoVehcile = true;   // TUM Used for sumo integration

    public float kp_control = 0.1f;
    public float ki_control = 0.0f;
    public float kd_control = 0.0f;

    SumoSocketClient sock;
    private PIController piControllerSpeed;
    private PIController piControllerDist;
    ...
    Vector2 RotateVectorByAngle(Vector2 v, float degrees) 
    {
    ...
    private void OnEnable() {
        ...
        // velocity controller
        piControllerDist = new PIController(15.0f, 0.000000f); // Example gains
        piControllerSpeed = new PIController(1.0f, 0.000000f); // Example gains

        pidControllerDist = new PIDController(15f, 0.000000f, 0.000000f); // Example gains
        pidControllerSpeed = new PIDController(2.0f, 0.000000f, 0.000000f); // Example gains
    ...
    /// <summary>
    /// SUMO inputs, not vehicle inputs.
    /// </summary>
    private void SumoInputs()
    {
        rbMarker = new Vector2(Rigid.position.x, Rigid.position.z);

        bool isInsidePhsyicsArea = Vehicle.SumoVehicleDetect(ref sock, id);

        // if(false){
        if(isInsidePhsyicsArea){
            float steeringGain = (float)0.05;

            // Just jump back into the phiysics world
            if (Rigid.isKinematic){
                Rigid.isKinematic = false; // this makes the vehicle move in the physics world
                var (steeringValue, torqueInput, desiredVelocity) = Vehicle.SumoVehicleControlWarmup(ref sock, id, Rigid, steeringGain, ref pidControllerSpeed, ref pidControllerDist, ref lookAheadMarker);
                // Debug.Log("The vehicle" + id + " with currentvelocity "+ Rigid.velocity.magnitude + " has applied real steeringValue: " + steeringValue + " torqueInput: " + torqueInput + " and desiredVelocity: " + desiredVelocity);
                steerInput_P = Mathf.Clamp(steeringValue, -1f, 1f);

                if (torqueInput > 0)
                {
                    throttleInput_P = torqueInput;
                    brakeInput_P = 0f;
                }
                else
                {
                    float brakeForceReduction = 1.0f;
                    throttleInput_P = 0f;
                    brakeInput_P = -torqueInput * brakeForceReduction;
                }
            ...
            // Stay in the physics world
            else{
                var (steeringValue, torqueInput, desiredVelocity) = Vehicle.SumoVehicleControl(ref sock, id, Rigid, steeringGain, ref pidControllerSpeed, ref pidControllerDist, ref lookAheadMarker);
                steerInput_P = Mathf.Clamp(steeringValue, -1f, 1f);
            ...
            Rigid = Vehicle.SumoVehicleTeleport(ref sock, id, Rigid, steeringGain, ref pidControllerSpeed, ref pidControllerDist, ref lookAheadMarker);           
            Rigid.isKinematic = true; // this makes the vehicle not move in the physics world
        ...
        //     string data = $"{timeStamp},{steerInput_P},{throttleInput_P},{brakeInput_P},{handbrakeInput_P},{clutchInput_P},{nosInput_P}";
